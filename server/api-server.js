...
// Store your sensitive info in .env files which you can grant server-only read access to. Since this is an 
// example app, the .env file is included for reference. If you re-use the code as a template, please use 
// your own database variables.
...
// Provide basic routing to fetch the posts from the MongoDB Database & send to the requestor
app.get('/posts/read', function (req, res) {
    client.connect(err => {
        client.db("blog").collection("posts").find({}).toArray(function(err, result) {
            if (err) {throw err;
            } else {
